...
type: skill
description: >
  High-water-mark-based tiered drawdown response with a persistent kill switch.
  Use when implementing or reviewing daily loss limits, HWM tracking, risk-tier
  scaling, and the capital protection gate that prevents catastrophic capital loss.
  If the kill switch fires, trading MUST stop — no exceptions.
---
