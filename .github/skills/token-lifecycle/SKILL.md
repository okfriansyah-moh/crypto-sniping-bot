...
type: skill
description: >
  Token lifecycle state machine patterns. Use when implementing or reviewing the
  token_lifecycle table, state transitions, CAS (compare-and-swap) updates, and
  transition validation. Every token's journey through the pipeline has a precise
  state — invalid transitions corrupt the audit trail.
---
...
- Config: `shared/config/pipeline.yaml` → `lifecycle.token_expire_sec`
