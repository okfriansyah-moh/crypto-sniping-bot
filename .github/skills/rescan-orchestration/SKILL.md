---
name: rescan-orchestration
type: skill
description: >
  Time-banded rescan orchestration. Use when implementing or reviewing any
  worker that periodically re-emits pipeline events for tokens at fixed age
  bands (15m / 30m / 45m / 1h after first detection) without altering
  downstream stages. Captures the Layer-0.5 rescan pattern: pure DB reader
  + event re-emitter, content-addressable EventID, mode-adaptive eligibility,
  structural-reject exclusion, open-position skip. Do NOT introduce new
  event types or new DTOs — re-emit existing market_data_event.
---
