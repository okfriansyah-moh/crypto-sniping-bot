---
name: strategy-auto-disable
type: skill
description: >
  Five-trigger lifecycle state machine for disabling degraded strategy versions.
  Use when implementing or reviewing the Learning Engine's automated strategy
  health enforcement (Layer 10). Strategies transition through:
  active → probation → degraded → disabled → review → re-enable.
  All transitions are logged and reversible. Disabled strategies are never deleted.
---
