...
- `shared/contracts/*.go` and `shared/contracts/*_test.go` already in the tree are **IMMUTABLE** outside Phase 0. If a DTO appears "wrong" relative to your test expectations, the test is wrong — not the DTO. Adjust the test, do not edit the contract.
- `shared/database/migrations/*.sql` are immutable. Test setup must use the existing migrations as-is.
- `docs/` is read-only.

When creating new tests, place them next to the module under test (`internal/modules/<name>/...`) or under `tests/`. Never create test files inside `shared/contracts/` for existing DTOs.
