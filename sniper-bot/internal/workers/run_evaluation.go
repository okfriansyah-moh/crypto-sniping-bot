...
import (
	...
	"crypto-sniping-bot/sniper-bot/internal/modules/evaluation"
...
func (w *EvaluationWorker) Process(ctx context.Context, evt *database.Event) (*database.Event, error) {
	...
	// Fetch shadow trades for the evaluation window (1h = 3600s).
	now := time.Now().UTC()
	rawShadows, shadowErr := w.adapter.GetShadowTradesByWindow(ctx, 3600)
	if shadowErr != nil {
