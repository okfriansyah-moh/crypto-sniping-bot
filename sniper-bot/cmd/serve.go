...
import (
	...
	"crypto-sniping-bot/sniper-bot/internal/modules/price_oracle"
...
func runServer() {
	...
	// Operator command consumer — dashboard POST /api/v1/commands → operator_command_event.
	go func() {
		if err := workers.RunOperatorCommands(ctx, db, logger); err != nil && err != ctx.Err() {
			logger.Error("operator_command_worker_failed", "error", err)
		}
	}()
