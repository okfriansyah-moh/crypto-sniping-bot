...
func buildStatusFn(db database.Adapter, cfg *config.Config, startTime time.Time) func(ctx context.Context) (string, error) {
	return func(ctx context.Context) (string, error) {
		overview, err := operator.BuildOverview(ctx, db, cfg, startTime)
		if err != nil {
		...
		if sv, svErr := db.GetActiveStrategyVersion(ctx); svErr == nil && sv != nil {
			versionLabel = sv.StrategyVersionID
		}
		overview.StrategyVersionID = versionLabel

		halted, haltReason, _ := db.IsSystemHalted(ctx)
		return formatTelegramStatus(overview, startTime, halted, haltReason), nil
	}
}

func newShadowGateEvaluator(db database.Adapter, cfg *config.Config) *health.ShadowGateEvaluator {
	return operator.NewShadowGateEvaluator(db, cfg)
}
...
func buildPnlFn(db database.Adapter) func(ctx context.Context) (string, error) {
	return func(ctx context.Context) (string, error) {
		summary, err := operator.BuildPnLSummary(ctx, db, closedPositionsLookbackSec/3600)
		if err != nil {
			return "", err
		}
		return formatTelegramPnL(summary), nil
	}
}

func buildPositionsFn(db database.Adapter) func(ctx context.Context) (string, error) {
	return func(ctx context.Context) (string, error) {
		positions, err := operator.ListOpenPositions(ctx, db, "")
		if err != nil {
			return "", err
		}
		return formatTelegramPositions(positions), nil
	}
}
...
func buildForceCloseFn(db database.Adapter, logger *slog.Logger) func(ctx context.Context, idOrAddr, issuer string) (string, error) {
	return func(ctx context.Context, idOrAddr, issuer string) (string, error) {
		...
		result, err := operator.ForceClosePositions(ctx, db, logger, issuer, idOrAddr, now, operator.CommandSourceTelegram, "")
		if err != nil {
			if strings.Contains(err.Error(), "no open position matches") {
				return fmt.Sprintf("No open position matches <code>%s</code>.", html.EscapeString(strings.TrimSpace(idOrAddr))), nil
			}
			if strings.Contains(err.Error(), "matches multiple tokens") {
				return fmt.Sprintf(
					"⚠️ Prefix <code>%s</code> matches multiple different tokens — give more characters.",
					html.EscapeString(strings.TrimSpace(idOrAddr)),
				), nil
			}
			return "", err
		}

		closedIDs := result.ClosedPositionIDs
...
func buildKillFn(db database.Adapter, logger *slog.Logger) func(ctx context.Context) error {
	return func(ctx context.Context) error {
		return operator.ExecuteKill(ctx, db, logger, "", operator.CommandSourceTelegram)
	}
}

func buildResumeFn(db database.Adapter, logger *slog.Logger) func(ctx context.Context) error {
	return func(ctx context.Context) error {
		return operator.ExecuteResume(ctx, db, logger, "", operator.CommandSourceTelegram)
	}
}
...
func buildModeFn(db database.Adapter, logger *slog.Logger) func(ctx context.Context, mode string) (string, error) {
	return func(ctx context.Context, mode string) (string, error) {
		if err := operator.ExecuteMode(ctx, db, logger, mode, "", operator.CommandSourceTelegram); err != nil {
			return "", err
		}
...
func buildPipelineFn(db database.Adapter) func(ctx context.Context) (string, error) {
	return func(ctx context.Context) (string, error) {
		stats, err := operator.FetchPipelineStats(ctx, db, 24)
		if err != nil {
			return "", err
		}
		return formatTelegramPipeline(stats), nil
	}
}
...
func buildDqFn(db database.Adapter) func(ctx context.Context, hours int) (string, error) {
	return func(ctx context.Context, hours int) (string, error) {
		report, err := operator.BuildDQTelegramReport(ctx, db, hours)
		if err != nil {
			return "", err
		}
		return formatTelegramDQ(report), nil
	}
}
