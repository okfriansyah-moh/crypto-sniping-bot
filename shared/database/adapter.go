...
type Adapter interface {
	...
	// ListRecentEvents returns the most recent event bus rows for the operator
	// activity feed. chain filters on events.chain with payload fallback; empty
	// chain returns all chains. limit is capped at 200 server-side (see
	// CapRecentEventsLimit). Read-only.
	ListRecentEvents(ctx context.Context, chain string, limit int) ([]RecentEventRow, error)

	// GetDQBreakdown returns data-quality decision counts and top reject reasons
	// for the dashboard DQ view. windowHours is clamped to [1, 168]. chain
	// filters data_quality rows and DQ_SKIPPED lifecycle rows; empty chain = all.
	// Read-only.
	GetDQBreakdown(ctx context.Context, windowHours int, chain string) (*DQBreakdown, error)
...
// RecentEventRow is a lightweight event bus tail entry for operator dashboards.
// Maps to contracts.ActivityEventDTO after summary formatting in internal/operator.
...
// DQRejectReasonCount is one row in a DQ reject-reason histogram.
type DQRejectReasonCount struct {
	Reason string
	Count  int64
}

// DQBreakdown aggregates data-quality decisions for the dashboard DQ view.
// SKIP counts come from token_lifecycle (DQ_SKIPPED); PASS/RISKY/REJECT from data_quality.
type DQBreakdown struct {
	WindowHours      int
	Chain            string
	TotalDecisions   int64
	PassCount        int64
	RiskyPassCount   int64
	RejectCount      int64
	SkipCount        int64
	PassRatePct      float64
...
func CapRecentEventsLimit(limit int) int {
	if limit <= 0 {
		return 50
	}
	if limit > 200 {
...
// CapDQWindowHours clamps dashboard DQ lookback to [1, 168] hours.
func CapDQWindowHours(windowHours int) int {
	if windowHours <= 0 {
		return 24
	}
	if windowHours > 168 {
