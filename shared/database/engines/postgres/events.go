...
// listRecentEventsSQL is the read-only tail query for operator activity feeds.
// Chain filter uses events.chain with JSON payload fallback for legacy rows.
const listRecentEventsSQL = `
SELECT
    e.event_id,
    e.event_type,
    COALESCE(NULLIF(e.chain, ''), COALESCE(e.payload->>'chain', '')) AS chain,
    COALESCE(e.payload->>'token_address', '') AS token_address,
    e.trace_id,
    e.created_at
FROM events e
WHERE ($1 = '' OR COALESCE(NULLIF(e.chain, ''), COALESCE(e.payload->>'chain', '')) = $1)
ORDER BY e.created_at DESC
LIMIT $2`

// ListRecentEvents returns recent event bus rows for the operator activity feed.
func (d *DB) ListRecentEvents(ctx context.Context, chain string, limit int) ([]database.RecentEventRow, error) {
	limit = database.CapRecentEventsLimit(limit)

	rows, err := d.pool.QueryContext(ctx, listRecentEventsSQL, chain, limit)
	if err != nil {
		return nil, fmt.Errorf("list recent events: %w", err)
	}
	defer rows.Close()

	out := make([]database.RecentEventRow, 0, limit)
	for rows.Next() {
		var row database.RecentEventRow
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("list recent events scan: %w", err)
		}
		row.CreatedAt = createdAt.UTC().Format(time.RFC3339Nano)
		out = append(out, row)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("list recent events rows: %w", err)
	}
	return out, nil
}
