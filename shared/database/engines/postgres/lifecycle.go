...
// dqBreakdownCountsSQL aggregates PASS/RISKY/REJECT from data_quality and SKIP
// from token_lifecycle (DQ_SKIPPED) within the lookback window.
const dqBreakdownCountsSQL = `
WITH dq AS (
    SELECT
        COUNT(*) FILTER (WHERE decision = 'PASS')       AS pass_count,
        COUNT(*) FILTER (WHERE decision = 'RISKY_PASS') AS risky_pass_count,
...
skip AS (
    ...
    FROM token_lifecycle tl
    INNER JOIN LATERAL (
        SELECT COALESCE(md.chain, '') AS chain
        FROM market_data md
        WHERE md.token_address = tl.token_address
        ORDER BY md.ingested_at DESC
        LIMIT 1
    ) md ON TRUE
    WHERE tl.current_state = 'DQ_SKIPPED'
      AND tl.updated_at >= NOW() - ($1 * INTERVAL '1 hour')
      AND ($2 = '' OR md.chain = $2)
)
SELECT dq.pass_count, dq.risky_pass_count, dq.reject_count, skip.skip_count
FROM dq, skip`

// dqTopRejectReasonsSQL unnests reject_reasons JSON arrays for REJECT rows.
const dqTopRejectReasonsSQL = `
SELECT reason, COUNT(*) AS cnt
FROM data_quality dq,
LATERAL jsonb_array_elements_text(dq.reject_reasons) AS reason
WHERE dq.decision = 'REJECT'
  AND dq.created_at >= NOW() - ($1 * INTERVAL '1 hour')
  AND ($2 = '' OR dq.chain = $2)
  AND reason <> ''
GROUP BY reason
...
func (d *DB) GetDQBreakdown(ctx context.Context, windowHours int, chain string) (*database.DQBreakdown, error) {
	...
	out := &database.DQBreakdown{
		WindowHours:      windowHours,
		Chain:            chain,
		TopRejectReasons: []database.DQRejectReasonCount{},
	}

	if err := d.pool.QueryRowContext(ctx, dqBreakdownCountsSQL, windowHours, chain).Scan(
		&out.PassCount,
		&out.RiskyPassCount,
		&out.RejectCount,
		&out.SkipCount,
	); err != nil {
		return nil, fmt.Errorf("get dq breakdown counts: %w", err)
	}

	out.TotalDecisions = out.PassCount + out.RiskyPassCount + out.RejectCount + out.SkipCount
	if out.TotalDecisions > 0 {
		out.PassRatePct = float64(out.PassCount+out.RiskyPassCount) / float64(out.TotalDecisions) * 100
	}

	rows, err := d.pool.QueryContext(ctx, dqTopRejectReasonsSQL, windowHours, chain)
	if err != nil {
		return nil, fmt.Errorf("get dq breakdown reject reasons: %w", err)
	}
	defer rows.Close()

	for rows.Next() {
		var item database.DQRejectReasonCount
		if err := rows.Scan(&item.Reason, &item.Count); err != nil {
			return nil, fmt.Errorf("get dq breakdown reject reasons scan: %w", err)
		}
		out.TopRejectReasons = append(out.TopRejectReasons, item)
	}
